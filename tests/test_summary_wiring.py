...
Covers:
- _start_summary_task() launches MultiDocSummaryWorker with correct params
...
- _poll_queue() checks summary worker liveness
- destroy() includes summary worker in stop/join cleanup
"""
...
class TestStartSummaryTask:
    """Test that _start_summary_task() correctly launches the worker."""

    def test_launches_worker_with_documents(self):
        """_start_summary_task() should create and start a MultiDocSummaryWorker."""
        ...
        with patch("src.services.workers.MultiDocSummaryWorker") as MockWorker:
            ...
            assert window._summary_worker is mock_instance

    def test_passes_correct_documents(self):
        """Worker should receive documents with filename and extracted_text."""
        from src.ui.main_window import MainWindow
    ...
    def test_prefers_preprocessed_text(self):
        ...
        with patch("src.services.workers.MultiDocSummaryWorker") as MockWorker:
            ...
            docs = MockWorker.call_args[1]["documents"]
            assert docs[0]["extracted_text"] == "clean text"

    def test_passes_ui_queue(self):
        """Worker should receive the UI queue for communication."""
        from src.ui.main_window import MainWindow
        ...
        with patch("src.services.workers.MultiDocSummaryWorker") as MockWorker:
            ...
            call_kwargs = MockWorker.call_args[1]
            assert call_kwargs["ui_queue"] is window._ui_queue

    def test_passes_ai_params_with_model(self):
        """Worker should receive ai_params including model_name."""
        from src.ui.main_window import MainWindow
        ...
        with patch("src.services.workers.MultiDocSummaryWorker") as MockWorker:
            ...
            ai_params = MockWorker.call_args[1]["ai_params"]
    ...
    def test_empty_documents_skips_worker(self):
        ...
        with patch("src.services.workers.MultiDocSummaryWorker") as MockWorker:
            ...
            assert window._summary_worker is None
...
class TestOnSummaryComplete:
    ...
    def test_clears_worker_reference(self):
        """Should set _summary_worker to None after completion."""
        from src.ui.main_window import MainWindow

        window = _make_mock_window()
        window._summary_worker = MagicMock()  # Simulate running worker
        result = self._make_result()
...
# -------------------------------------------------------------------------
# Polling liveness tests (source inspection)
# -------------------------------------------------------------------------


class TestSummaryPollingIntegration:
    """Test that _poll_queue checks summary worker liveness."""

    def test_poll_queue_checks_summary_worker(self):
        """_poll_queue source should check _summary_worker.is_alive()."""
        from src.ui.main_window import MainWindow

        source = inspect.getsource(MainWindow._poll_queue)
        assert "_summary_worker" in source
        assert "summary_alive" in source

    def test_poll_queue_re_check_includes_summary(self):
        """Post-drain re-check should look for newly started summary workers."""
        from src.ui.main_window import MainWindow

        source = inspect.getsource(MainWindow._poll_queue)
        assert "new_summary_alive" in source


# -------------------------------------------------------------------------
# destroy() cleanup tests (source inspection)
# -------------------------------------------------------------------------


class TestSummaryDestroyCleanup:
    """Test that destroy() includes summary worker in cleanup."""

    def test_destroy_includes_summary_worker(self):
        """destroy() should include summary worker in workers_to_stop list."""
        from src.ui.main_window import MainWindow

        source = inspect.getsource(MainWindow.destroy)
        assert '"summary"' in source
        assert "_summary_worker" in source

    def test_summary_worker_initialized_in_init(self):
        """__init__ should initialize _summary_worker = None."""
        from src.ui.main_window import MainWindow
