...
class TestTitlePageRemover:
    ...
    def test_whole_page_removed_no_mid_page_split(self):
        """Title page is removed whole; no mid-page content splitting."""
        title = (
            "SUPREME COURT OF THE STATE OF NEW YORK\n"
            "COUNTY OF KINGS\n"
            "Index No. 123456/2024\n"
        ...
        # Entire title page removed — none of it bleeds into kept text
        assert "SUPREME COURT" not in result.text
        assert "JOHN DOE, Plaintiff" not in result.text
        assert "Q. What happened?" in result.text

    def test_stops_at_first_non_title_page(self):
        """Once a page does not score as a title, all subsequent pages are kept."""
        title = (
            "SUPREME COURT\nIndex No. 123\nPLAINTIFF, -against- DEFENDANT.\n"
            "DEPOSITION OF JOHN DOE\n"
        )
        # Page 2: no title patterns — should stop removal
        content_page = "Q. Name?\nA. John.\nQ. Age?\nA. Forty."
        # Page 3: has title-like patterns but should NOT be removed (already past threshold)
        page3 = "SUPREME COURT\nIndex No. 999\nThis page has title patterns."
        text = title + "\f" + content_page + "\f" + page3
        result = self._make().process(text)

        assert "SUPREME COURT\nIndex No. 999" in result.text
