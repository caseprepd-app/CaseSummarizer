...
class TestDynamicOutputWidgetWorkflowMethods:
    ...
    def test_set_workflow_phase_updates_phase(self, dynamic_output_widget):
        ...
        dynamic_output_widget.set_workflow_phase(WorkflowPhase.SEMANTIC_SEARCHING)
        assert dynamic_output_widget._workflow_phase == WorkflowPhase.SEMANTIC_SEARCHING
    ...
    def test_set_workflow_phase_updates_labels(self, dynamic_output_widget):
        ...
        qa_text = dynamic_output_widget._semantic_status_label.cget("text")
    ...
    def test_set_tab_status_config_updates_qa(self, dynamic_output_widget):
        """set_tab_status_config should update semantic_enabled."""
        dynamic_output_widget.set_tab_status_config(semantic_enabled=False)
        assert dynamic_output_widget._tab_status_config.semantic_enabled is False
    ...
    def test_set_tab_status_config_partial_update(self, dynamic_output_widget):
        ...
        # Set initial state
        dynamic_output_widget.set_tab_status_config(vocab_enabled=False, semantic_enabled=False)

        # Update only one value
        dynamic_output_widget.set_tab_status_config(semantic_enabled=True)

        # Check that other values are preserved
        config = dynamic_output_widget._tab_status_config
        assert config.vocab_enabled is False  # Preserved
        assert config.semantic_enabled is True  # Updated

    def test_show_semantic_content_hides_status_label(self, dynamic_output_widget):
        """show_semantic_content should hide status label and show panel."""
        dynamic_output_widget.show_semantic_content()

        # Status label should be hidden (grid_remove)
        assert not dynamic_output_widget._semantic_status_label.winfo_ismapped()

    def test_show_semantic_status_shows_status_label(self, dynamic_output_widget):
        """show_semantic_status should show status label and hide panel."""
        # First show content, then show status
        dynamic_output_widget.show_semantic_content()
        dynamic_output_widget.show_semantic_status()

        # Status label should be visible
        assert not dynamic_output_widget._semantic_panel.winfo_ismapped()
...
class TestWorkflowPhaseTransitions:
    ...
    def test_vocab_to_semantic_indexing(self, dynamic_output_widget):
        """Transition from VOCAB_RUNNING to SEMANTIC_INDEXING."""
        from src.ui.workflow_status import WorkflowPhase

        dynamic_output_widget.set_workflow_phase(WorkflowPhase.VOCAB_RUNNING)
        dynamic_output_widget.set_workflow_phase(WorkflowPhase.SEMANTIC_INDEXING)

        qa_text = dynamic_output_widget._semantic_status_label.cget("text")
        assert "building search index from your documents" in qa_text

    def test_semantic_indexing_to_searching(self, dynamic_output_widget):
        """Transition from SEMANTIC_INDEXING to SEMANTIC_SEARCHING."""
        from src.ui.workflow_status import WorkflowPhase
...
class TestStatusLabelVisibility:
    ...
    def test_semantic_status_label_exists(self, dynamic_output_widget):
        """Q&A status label should exist."""
        assert dynamic_output_widget._semantic_status_label is not None
    ...
    def test_semantic_panel_initially_hidden(self, dynamic_output_widget):
    ...
    def test_toggle_semantic_visibility(self, dynamic_output_widget):
        """Should be able to toggle semantic content visibility."""
        # Show content
        dynamic_output_widget.show_semantic_content()
        assert not dynamic_output_widget._semantic_status_label.winfo_ismapped()
...
class TestConfigAndPhaseInteraction:
    ...
    def test_vocab_disabled_affects_idle_message(self, dynamic_output_widget):
        ...
        dynamic_output_widget.set_tab_status_config(vocab_enabled=False, semantic_enabled=True)
        dynamic_output_widget.set_workflow_phase(WorkflowPhase.IDLE)
