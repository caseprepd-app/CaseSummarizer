...
Shows a plain-English breakdown of why the ML model scored a term
the way it did. Opened via right-click "Why this score?" on a term
...
class ScoreExplanationDialog(BaseModalDialog):
    ...
    contributing factors in plain English with colored +/- indicators.
    ...
    def _create_ui(self):
        ...
        if reasons:
            ...
            for label, contrib in reasons:
                self._add_reason_row(scroll, label, contrib)
        else:
        ...
        model_text = (
            ...
            else "Based on LR + Random Forest ensemble"
        )
    ...
    def _add_reason_row(self, parent_frame, label: str, contribution: float):
        """
        Add a single reason row with +/- indicator.
